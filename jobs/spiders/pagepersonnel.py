...
class PagepersonnelSpider(scrapy.Spider):
    name = 'pagepersonnel'
    allowed_domains = ['www.pagepersonnel.com.hk']
    start_urls = [
        'https://www.pagepersonnel.com.hk/jobs?sort_by=most_recent&page-index=0&page=0']
    ...
    def parse(self, response):
        ...
        jobs_url = response.xpath(
            '//*[@class="view-content"]/ul//li//a/@href').getall()
        ...
        for job_url in jobs_url:
            ...
            job_url = "https://www.pagepersonnel.com.hk" + job_url
            print("Now working on JobURL", job_url)

            yield scrapy.Request(job_url, callback=self.parse_job)

        pages = response.xpath(
            '//*[@class="pager-show-more-next first last"]/a/@href').get()
    ...
    def parse_job(self, response):
        ...
        jj = json.loads(response.xpath(
            '//script[@type="application/ld+json"]//text()').extract_first(), strict=False)
        ...
        job_item.add_value('source', 'pagepersonnel')
        ...
        job_func_str = response.xpath('//span[contains(text(),"Function:")]/following-sibling::span/a/span/text()').extract()[0] + " ( " + response.xpath('//span[contains(text(),"Specialisation:")]/following-sibling::span/a/text()').extract()[0] + ")"
        job_item.add_value('function', [job_func_str])
